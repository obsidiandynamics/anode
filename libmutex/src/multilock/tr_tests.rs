...
use crate::multilock::{Fairness, LockReadGuard, LockWriteGuard, UpgradeOutcome};
...
fn micro_bench() {
    for fairness in FAIRNESS_VARIANTS {
        __micro_bench(fairness.into());
    }
}

fn __micro_bench(fairness: Fairness) {
    ...
    let protected = Arc::new(MultiLock::new(0, fairness));

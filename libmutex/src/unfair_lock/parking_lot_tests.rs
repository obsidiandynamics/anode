...
fn test_rw_arc_no_poison_wr() {
    let arc = Arc::new(UnfairLock::new(1));
    let arc2 = arc.clone();
...
fn test_rw_arc_with_chalice() {
    let arc = Arc::new(UnfairLock::new(Chalice::new(1)));
    let arc2 = arc.clone();
    let _ = thread::spawn(move || {
...
#[test]
fn test_ruw_arc() {
    let arc = Arc::new(UnfairLock::new(0i32));
    let arc2 = arc.clone();
    let (tx, rx) = channel();
...
#[test]
fn test_rw_arc() {
    let arc = Arc::new(UnfairLock::new(0));
    let arc2 = arc.clone();
    let (tx, rx) = channel();
...
fn test_rwlock_unsized() {
    let rw: &UnfairLock<[i32]> = &UnfairLock::new([1, 2, 3]);
    {
        let b = &mut *rw.write();
        b[0] = 4;
        b[2] = 5;
...
fn test_rwlock_try_read() {
    let lock = UnfairLock::new(0isize);
    {
...
#[test]
fn test_into_inner() {
    let m = UnfairLock::new(NonCopy(10));
...
fn test_into_inner_drop() {
    ...
    let m = UnfairLock::new(Foo(num_drops.clone()));
...
#[test]
fn test_get_mut() {
    let mut m = UnfairLock::new(NonCopy(10));
...
fn test_rwlock_downgrade() {
    let x = Arc::new(UnfairLock::new(0));
...
fn test_rwlock_debug() {
    let x = UnfairLock::new(());
    assert!(format!("{:?}", x).contains("UnfairLock"));
}
...
fn test_issue_203() {
    ...
    thread_local! {
        static B: Bar = Bar(UnfairLock::new(()));
    }

    thread::spawn(|| {
        B.with(|_| ());

        let a = UnfairLock::new(());
        let _a = a.read();
    })
    .join()
    .unwrap();

...
fn unbounded_execute_tasks_via_try_submit() {
    ...
    for threads in THREADS {
        ...
        let tasks = (0..TASKS)
            .map(|_| {
                let submission = submitter.try_submit(|| {});
                submission.unwrap()
            })
            .collect::<Vec<_>>();
        ...
        let tasks = (0..TASKS)
            .map(|_| {
                ...
                while submission.is_none() {
                    submission = submitter.try_submit(|| {});
                }
                submission.unwrap()
            })
            .collect::<Vec<_>>();
...
fn unbounded_abort_from_submitter() {
    ...
    let task_2 = submitter.submit(|| {});
    println!("submitted task_2");
    let task_3 = submitter.submit(|| {});
...
fn bounded_queuing() {
    ...
    let task_2 = submitter.try_submit(|| {}).unwrap();
    println!("submitted task_2");
    assert!(!task_1.is_complete());

    // task_3 cannot be submitted at this stage
    let task_3 = submitter.try_submit(|| {});
